...
namespace singleton {
	...
	const uint32_t uart_device::ranks[10] = { 1000000000, 100000000, 10000000, 1000000, 100000, 10000, 1000, 100, 10, 1 };
	...
	uint8_t uart_device::read_block(uint8_t* i, uint8_t size) {
		for(uint8_t j{0}; j < size; ++j) {
			*i++ = read();
		}
	}

void uart_device::write_endl() {
		write('\r');
		write('\n');
	}
	...
	void uart_device::write_hex_block(uint8_t* i, uint8_t size, uint8_t rows) {
		for(uint8_t j{0}; j < size; ++j) {
			write_hex(*i++);
			if((j + 1) % rows) {
				write(' ');
