...
namespace singleton {
	...
	struct uart_device {	// Truth is, singletons and inheritance do not play well together.
		...
		uint8_t read_block(uint8_t* i, uint8_t size);
		...
		/**
		 * write a block of hex spaced every byte 
		 */
		void write_hex_block(uint8_t* i, uint8_t size, uint8_t rows);
	...
	private:
		...
		*/
		...
		static const uint32_t ranks[10];
